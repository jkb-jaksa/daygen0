...
// TEMPORARILY DISABLED: Authentication guard to allow unauthenticated image generation
...
export class ImageGenerationController {
  ...
  async generateGemini(
    @CurrentUser() user: SanitizedUser | undefined,
  ...
  private enqueueImageJob(
    ...
    // Use 'anonymous' as userId for unauthenticated requests
    const userId = user?.authUserId ?? 'anonymous';

    return this.cloudTasksService.createImageGenerationJob(userId, {
      prompt: dto.prompt,
      model,
      provider,
      options: dto,
