...
import QuickEditModal, { type QuickEditOptions } from '../create/QuickEditModal';
import type { StoredAvatar } from './types';

interface CreateImageAvatarModalProps {
    isOpen: boolean;
    onClose: () => void;
    avatar: StoredAvatar;
...
/**
 * Avatar-specific image creation modal.
 * Wraps QuickEditModal with a custom overlay showing avatar name and image thumbnails.
 */
const CreateImageAvatarModal: React.FC<CreateImageAvatarModalProps> = ({
    isOpen,
    onClose,
    avatar,
    onSubmit,
    ...
    const handleSubmit = useCallback((options: QuickEditOptions) => {
        onSubmit({
            ...options,
            avatarId: avatar.id,
...
export default CreateImageAvatarModal;
