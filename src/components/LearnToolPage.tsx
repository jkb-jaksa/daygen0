            ...
            <header className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                {toolContent.categories.map((category) => (
                  <span
                    key={category}
                    className="inline-flex items-center rounded-full border border-theme-dark/60 bg-theme-black/40 px-3 py-1 text-xs font-raleway uppercase tracking-[0.25em] text-theme-white/70"
                  >
            ...
            <div className="mt-10 grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]">
              <div className="space-y-6">
                <section className={sectionCardStyles}>
                  <h2 className="text-xl font-raleway font-medium text-theme-text">Overview</h2>
                  <p className="mt-3 text-sm font-raleway leading-relaxed text-theme-white">
                    {toolContent.overview}
                  </p>
                </section>

                <section className={sectionCardStyles}>
                  <h2 className="text-xl font-raleway font-medium text-theme-text">Use cases</h2>
                  {hasUseCases ? (
                    <ul className="mt-3 space-y-2 text-sm font-raleway text-theme-white/90">
                      {toolContent.useCases.map((item) => (
                        <li key={item} className="flex items-start gap-2">
                          <span className="mt-1 h-1.5 w-1.5 rounded-full bg-theme-white/70" aria-hidden="true" />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="mt-3 text-sm font-raleway text-theme-white/70">
                ...
                <section className={sectionCardStyles}>
                  <h2 className="text-xl font-raleway font-medium text-theme-text">Key features</h2>
                  {hasFeatures ? (
                    <ul className="mt-3 space-y-4 text-sm font-raleway text-theme-white/90">
                      {toolContent.features.map((feature) => (
                        <li key={feature.title}>
                          <p className="text-base font-raleway font-medium text-theme-text">{feature.title}</p>
                          <p className="mt-1 text-sm font-raleway text-theme-white/80">{feature.description}</p>
                        </li>
                      ))}
                    </ul>
                  ) : (
              ...
              <aside className="space-y-6">
                <section className={sectionCardStyles}>
                  <h2 className="text-xl font-raleway font-medium text-theme-text">Pricing snapshot</h2>
                  <p className="mt-3 text-sm font-raleway leading-relaxed text-theme-white/90">
                    {toolContent.pricing}
                  </p>
                </section>

                <section className={sectionCardStyles}>
                  <h2 className="text-xl font-raleway font-medium text-theme-text">Getting started</h2>
                  {hasGettingStarted ? (
                    <ol className="mt-3 list-decimal space-y-2 pl-5 text-sm font-raleway text-theme-white/90">
                ...
                <section className={sectionCardStyles}>
                  <h2 className="text-xl font-raleway font-medium text-theme-text">Resources</h2>
                  {hasResources ? (
                    <ul className="mt-3 space-y-2 text-sm font-raleway text-theme-white/90">
                      {toolContent.resources?.map((resource) => (
                        <li key={resource.href}>
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="inline-flex items-center gap-2 text-theme-white/80 transition-colors duration-150 hover:text-theme-text"
                          >
                            {resource.label}
