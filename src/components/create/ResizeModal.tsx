...
import { X, Scaling, Image as ImageIcon, Move, ZoomIn, ZoomOut, RotateCcw, Sparkles } from 'lucide-react';
...
const ResizeModal = memo<ResizeModalProps>(({
    ...
    const handleDragMove = useCallback((e: MouseEvent | TouchEvent) => {
        if (!isDragging || !dragStartRef.current || !canvasRef.current || !layoutInfo) return;
        const clientX = 'touches' in e ? e.touches[0].clientX : e.clientX;
        const clientY = 'touches' in e ? e.touches[0].clientY : e.clientY;
    ...
    const isSameRatio = layoutInfo?.type === 'same';
    // Check if there is any AI fill needed (scale < 100 or not covering full canvas)
    const needsAiFill = imageScale < 100 || !isSameRatio;
    ...
    const renderCanvas = () => {
        ...
        // No aspect ratio selected yet - show original
        if (!selectedAspectRatio || !layoutInfo) {
            return (
                        ...
                        <>
                            ...
                            <div className="mt-auto pt-6">
                                ...
                                >
                                    ...
                                    {isSameRatio && imageScale === 100 ? 'Keep Current Size' : needsAiFill ? 'Generate & Extend' : 'Resize Image'}
                                </button>
                            </div>
                        </>
                    </div>
